        ...
        public IActionResult Download()
        {
            ...
            using (var package = new ExcelPackage())
            {
                ...
                for (int r = 2; r <= totalRows + 1; r++)
                {
                    ...
                    {
                        worksheet.Cells[row, 11].Value = "cert 2";
                        worksheet.Cells[row, 12].Value = employeeList[i].Cert2StartDate;
                        worksheet.Cells[row, 12].Style.Numberformat.Format = "mm/dd/yyyy";
                        worksheet.Cells[row, 13].Value = employeeList[i].Cert2EndDate;
                        worksheet.Cells[row, 13].Style.Numberformat.Format = "mm/dd/yyyy";
                        row++;
                    }
                    if (employeeList[i].RequiredCerts.Contains("3"))
                    {
                        worksheet.Cells[row, 11].Value = "cert 3";
                        worksheet.Cells[row, 12].Value = employeeList[i].Cert3StartDate;
                        worksheet.Cells[row, 12].Style.Numberformat.Format = "mm/dd/yyyy";
                        worksheet.Cells[row, 13].Value = employeeList[i].Cert3EndDate;
                        worksheet.Cells[row, 13].Style.Numberformat.Format = "mm/dd/yyyy";
                        row++;
                    }
                    i++;
